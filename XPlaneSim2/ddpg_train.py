...
from env2 import XplaneEnv as XPlane
# from xplane_envBase import XplaneEnv as XPlane
import general_utilities


def play(episodes, max_steps, is_testing, checkpoint_interval,
    ...
    for episode in range(episodes):
        ...
        while True:
            ...
            for i in range(env.n):
                ...
                #     actors[i].choose_action(states),-1,1)
                # print(actors[i].choose_action(states))
                action=actors[i].choose_action(states)
                actions=action
                print(actions)
            ...
            if  is_testing== True:
                ...
                for i in range(env.n):
                    ...
                    memories[i].remember(states, actions,
        ...
        print("steps: {}, episodes: {}, episode reward: {}, time: {} \n".format(
            cum_steps, episode, episode_rewards, round(time.time() - t_start, 3)))
    return statistics
...
if __name__ == '__main__':
    ...
    parser.add_argument("--max_steps", type=int, default=200, help="maximum steps length")  # 每个episode的步数为400步
    ...
    parser.add_argument('--weights_filename_prefix', default='/save/xplane-ddpg',
    ...
    if args.ou_mus is not None:
        if len(args.ou_mus) == len(env.act_space):
            ou_mus = []
            prev_idx = 0
            for space in env.act_space:
                ou_mus.append(
        ...
        ou_mus=[]
        ou_mus.append(len(env.act_space))
    ...
    for i in range(env.n):
        ...
        action_bound=1
        actors.append(Actor('actor' + str(i), session, n_action, action_bound,
                            state, state_next))
    ...
    # play
    statistics = play(args.episodes, args.max_steps, args.testing,
