...
class XplaneEnv(gym.Env):
    ...
    def __init__(self, clientAddr='0.0.0.0', xpHost='127.0.0.1', xpPort=49009, clientPort=1, timeout=100):
        # CLIENT = client
        ...
        self.observation_space = spaces.Box(low=np.array([ -360, -360, -360 ,0,0,0,0,0]),high=np.array([360,360,360,360,360,360,5000,1000]), dtype=np.float32)
    ...
    def rewardCalcul(self, targetState, xplaneState):
        '''
        ...
        #  roll reward
        reward += 1 if abs(xplaneState[1]-targetState[1])<=10 else -1
        # pitch reward
        reward += 1 if abs(xplaneState[2]-targetState[2])<=10 else -1
        # yaw reward
        reward += 1 if abs(xplaneState[3]-targetState[3])<=10 else -1
        # altitude reward
        reward += 1 if abs(xplaneState[4] - targetState[4]) <= 10 else -1
        # airspeed reward
        reward += 1 if abs(xplaneState[5] - targetState[5]) <= 10 else -1
        data = np.array([targetState, xplaneState])
        pairwise_dists = pdist(data, 'cosine')  # 计算向量targetState和xplaneState之间的余弦距离
        # print(pairwise_dists)
        if pairwise_dists[0] < 0.01:
            self.ControlParameters.flag = True
            reward += 100
        reward -= pairwise_dists*1000
        return reward
    ...
    def step(self, actions):
        ...
        *Gear(0 = up, 1 = down)
        ...
        reward = self.rewardCalcul(targetState, xplaneState)
        # print(reward)
        self.ControlParameters.episodeReward += reward
        ###########################################################################
        # 如果坠机
        if XplaneEnv.CLIENT.getDREFs(self.ControlParameters.crash)[0][0] <=0:
    ...
    def reset(self):
        ...
        pos=[0,0,5000,np.random.uniform(-5,5),np.random.uniform(-5,5),np.random.uniform(-5,5),-998]
