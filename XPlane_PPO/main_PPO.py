...
def main():
    ...
    HIDDEN_DIM = 64
    agent = PPO(s_dim, a_dim, a_bound, HIDDEN_DIM,args.learning_rate, args.learning_rate)
    ...
    if not args.testing:
        for episode in range(args.episodes):
            ...
            for t in range(max_steps):
                ...
                if (t + 1) % args.batch_size == 0 or t == max_steps - 1:
                    agent.finish_path(observation, done)
                    agent.update()
            ...
            if (episode+1) % args.checkpoint_frequency ==0:
                ...
                agent.save_ckpt(args.saveModel_dir)
