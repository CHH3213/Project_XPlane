...
class PPO(object):
    ...
    PPO 类
    '''
    def __init__(self, state_dim, action_dim, action_bound, actorLR=A_LR, criticLR=C_LR, method='clip'):
        ...
        def build_actor(input_state_dim, action_dim):
            ''' actor 网络，输出mu和sigma '''
            input_layer = tl.layers.Input(input_state_dim, tf.float32)
            # input_layer = tl.layers.Dropout(input_layer, keep=0.8, name="drop_layer")
            l1 = tl.layers.Dense(100, tf.nn.relu)(input_layer)
        ...
        self.actor.train()
        self.actor_old.eval()
        self.actor_opt = tf.optimizers.Adam(actorLR)
        self.critic_opt = tf.optimizers.Adam(criticLR)
    ...
    #     print("load weight!")
    def save_ckpt(self,path):  # save trained weights
        # path = os.path.join('model', '_'.join([ALG_NAME, ENV_ID]))
        if not os.path.exists(path):
            os.makedirs(path)
        extend_path = lambda s: os.path.join(path, s)
        tl.files.save_npz( self.actor.trainable_weights, extend_path('actor.npz'))
        tl.files.save_npz(self.actor_old.trainable_weights, extend_path('actor_old.npz'))
        tl.files.save_npz(self.critic.trainable_weights, extend_path('critic.npz'))


    def load_ckpt(self,path):  # load trained weights
        # path = os.path.join('model', '_'.join([ALG_NAME, ENV_ID]))
        extend_path = lambda s: os.path.join(path, s)
        tl.files.load_and_assign_npz(extend_path('actor.npz'), self.actor)
        tl.files.load_and_assign_npz(extend_path('actor_old.npz'), self.actor_old)
        tl.files.load_and_assign_npz(extend_path('critic.npz'), self.critic)
    ...
    if args.train:
        ...
        for episode in range(EP_MAX):
            ...
            for t in range(EP_LEN):
                ...
                ppo.store_transition(state, action, (reward + 8) / 8, done)
                state = state_
