...
# import tensorflow.compat.v1 as tfp
# tfp.disable_eager_execution()  # 可以用于从TensorFlow 1.x到2.x的复杂迁移项目的程序开头
...
class ReplayBuffer:
    ...
    def push(self, state, action, reward, next_state, done):
        ...
        self.position = int((self.position + 1) % self.capacity)  # as a ring buffer capacity一般为1e6

    def sample(self, batch_size):
        batch = random.sample(self.buffer, batch_size)  # 截取self.buffer的指定长度的随机数，但是不会改变self.buffer本身的排序：
...
class QNetwork(Model):
    ...
    def __init__(self, num_inputs, num_actions, hidden_dim, init_w=3e-3):
        super(QNetwork, self).__init__()  # 这是对继承自父类的属性进行初始化。而且是用父类的初始化方法来初始化继承的属性。
        input_dim = num_inputs + num_actions  # state+action
        w_init = tf.random_uniform_initializer(-init_w, init_w)  # 生成均匀分布张量初始化器的类。
        # print('w_init', type(w_init))
...
class PolicyNetwork(Model):
    ...
    def __init__(self, num_inputs, num_actions, hidden_dim, action_range=1., init_w=3e-3):
        ...
        self.output_linear = Dense(
            ...
            in_channels=hidden_dim, name='policy_output',act=tf.nn.tanh
        )
...
class TD3:
    ...
    def __init__(
        ...
        self.q_net1.train()
        self.q_net2.train()
        self.target_q_net1.eval()  # 类似sess.run
        self.target_q_net2.eval()
        # print(type(self.target_q_net1.eval()))
    ...
    def target_ini(self, net, target_net):
        """ hard-copy update for initializing target networks --初始化时硬更新"""
    ...
    def update(self, batch_size, eval_noise_scale, reward_scale=10., gamma=0.9, soft_tau=1e-2):
        ...
        reward = reward[:, np.newaxis]  # expand dim 扩展一个纬度：例如(5, 1)--> (5, 1, 1)
        ...
        reward = reward_scale * (reward - np.mean(reward, axis=0)) / (
            np.std(reward, axis=0) + 1e-6  # axis=0计算每一列的标准差  reward归一化
        ...
        q_input = tf.concat([state, action], 1)  # input of q_net 张量的拼接
        # print('q_input', q_input.numpy())
        # print('q_input', q_input.shape)  # (64, 4)
        # 基于loss计算梯度，并进行反向误差传播，更新网络权重。
        with tf.GradientTape() as q1_tape:
            predicted_q_value1 = self.q_net1(q_input)
            q_value_loss1 = tf.reduce_mean(tf.square(predicted_q_value1 - target_q_value))
            # print('q_value_loss1', q_value_loss1.numpy())
        ...
        # 其中update_cnt是更新的次数，policy_target_update_interval是policy网络更新的周期，
        # 每当critic更新了一定次数后，再更新policy网络。
        if self.update_cnt % self.policy_target_update_interval == 0:
            with tf.GradientTape() as p_tape:
                ...
                # print('state',state.shape)  # shape=(64, 3)
                # print('new_action',new_action)  # shape=(64, 1), dtype=float32
                # print('new_q_input',new_q_input)  # shape=(64, 4), dtype=float32
                """ implementation 1 """
    ...
    def save(self,path):  # save trained weights
    ...
    def load(self,path):  # load trained weights
        # path = os.path.join('model', '_'.join([ALG_NAME, ENV_ID]))
        extend_path = lambda s: os.path.join(path, s)
