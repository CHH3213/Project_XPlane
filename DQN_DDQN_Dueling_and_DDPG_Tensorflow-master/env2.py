...
class XplaneEnv(gym.Env):
    ...
    def __init__(self, clientAddr='0.0.0.0', xpHost='127.0.0.1', xpPort=49009, clientPort=1, timeout=100):
        ...
        self.observation_space = spaces.Box(low=np.array([ -360, -360, -360 ,-180,-180,-180,0,0]),high=np.array([360,360,360,180,180,180,5000,1000]), dtype=np.float32)
    ...
    def rewardCalcul(self, targetState, xplaneState,action):
        ...
        pairwise_dists = pdist(data, 'cosine')  # 计算向量targetState和xplaneState之间的余弦距离，1-pairwise_dists表示相似度
        # print(pairwise_dists)
        # if pairwise_dists[0] < 0.01:
        #     self.ControlParameters.flag = True
        #     reward += 100
        reward -= pairwise_dists * 100
