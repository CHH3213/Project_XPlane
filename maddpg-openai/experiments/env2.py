...
class XplaneEnv(gym.Env):
    ...
    def __init__(self, clientAddr, xpHost, xpPort, clientPort, timeout=3000, max_episode_steps=1000, test=False):
        # CLIENT = client
    ...
    def rewardCalcul(self, target_state, xplane_state, sigma=0.45):
        ...
        pairwise_dists = pdist(data, 'cosine')  # 计算向量target_state和xplane_state之间的余弦距离
    ...
    def step(self, actions):
        ...
        reward = 0.
        perturbationAllowed = [3.5, 15]  # 允许的高度和航向的偏移量
        actions_ = []
        ...
        try:
            ...
            *Gear(0 = up, 1 = down)
            ...
            # combine the position and other state parameters in temporary variable here--10维
            ...
            # this is set to motivate the agent to move forward in time . Accumulate disance
            ...
            P = XplaneEnv.CLIENT.getDREF("sim/flightmodel/position/P")[0][0]  # The roll rotation rates (relative to the flight)
            Q = XplaneEnv.CLIENT.getDREF("sim/flightmodel/position/Q")[0][0]  # The pitch rotation rates (relative to the flight)
            R = XplaneEnv.CLIENT.getDREF("sim/flightmodel/position/R")[0][0]  # The yaw rotation rates (relative to the flight)
            ##############################################################################
            ...
            if len(state) == self.statelength:  # this should be true if len(state) is 10
                ...
                self.ControlParameters.state11['roll_rate'] = P  # The roll rotation rates (relative to the flight)
                self.ControlParameters.state11['pitch_rate'] = Q  # The pitch rotation rates (relative to the flight)
                self.ControlParameters.state11['altitude'] = state[2]  # Altitude
                self.ControlParameters.state11['Pitch'] = state[3]  # pitch
                self.ControlParameters.state11['Roll'] = state[4]  # roll
                self.ControlParameters.state11['velocity_x'] = state[6]  # local velocity x  OpenGL coordinates
                self.ControlParameters.state11['velocity_y'] = state[7]  # local velocity y  OpenGL coordinates
                self.ControlParameters.state11['velocity_z'] = state[8]  # local velocity z   OpenGL coordinates--右手坐標係
                self.ControlParameters.state11['delta_altitude'] = abs(state[2] - minimumAltitude)  # difference in altitude
                self.ControlParameters.state11['delta_heading'] = abs(state[5] - headingReward)  # difference in heading
                self.ControlParameters.state11['yaw_rate'] = R  # The yaw rotation rates (relative to the flight)
                if self.test:
                    ...
                    state11 = state  # state variable this inclue lat long for ploting

                else:
                    # lat long have been overriden. The dictionary above is used as normal during training
                    state11 = [i for i in self.ControlParameters.state11.values()]
            ###########################################################################
            # *******************************reward computation ******************
            ...
            # if crash, reward add -3 otherwise reward remain same
            # self.ControlParameters.on_ground:int[10]
            ...
            self.ControlParameters.state11 = self.ControlParameters.state11
            if self.test:
                state.append(0)
                state11 = state
            else:
        ...
        return np.array(state11), reward, self.ControlParameters.flag, self._get_info()  # self.ControlParameters.state11
    ...
    def reset(self):
        ...
        # self.episode_steps = 0
        self.ControlParameters.episodeStep = 0
        self.ControlParameters.state11 = dict.fromkeys(self.ControlParameters.state11.keys(), 0)
        # print(self.ControlParameters.state11)
        # stater  = [0,4]
        ...
        return state  # self.ControlParameters.state11
