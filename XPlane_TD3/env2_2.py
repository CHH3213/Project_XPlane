...
class XplaneEnv(gym.Env):
    ...
    def __init__(self, clientAddr='0.0.0.0', xpHost='127.0.0.1', xpPort=49009, clientPort=1, timeout=10000):
        # CLIENT = client
    ...
    def rewardCalcul(self, norm_targetState, xplaneState, norm_xplaneState, action):
        ...
        posi = np.array(xplaneState[3:5] + [xplaneState[7]])/400
        print(posi)
        dist1 = distance.euclidean(posi, np.array([0, 3 / 180,80/400]))

        act_m = np.array([[xplaneState[0] / 200, xplaneState[3] / 180],
                          [xplaneState[1] / 200, xplaneState[4] / 180]])
        act = -np.einsum('ij,j->i', act_m, np.array([1, 3]))
        act[0] = np.clip(act[0], -1, 1)
        act[1] = np.clip(act[1], -1, 1)
        true_act = np.array([0,0])
        true_act[0] = np.clip(action[1],-1,1)
        true_act[1] = np.clip(action[0],-1,1)
        dist2 = distance.euclidean([action[1], action[0]], act)

        reward += -dist1*5 - dist2*0.4
        # print(reward, dist1, dist2, self.success_count,act)
        # print(xplaneState[3:5], act)
        if reward > -0.9:
            self.success_count += 1
            reward += self.success_count
        elif -3 < reward <= -1.5:
            if self.success_count > 0:
                self.success_count -= 1
                reward -= self.success_count*0.8
            else:
                reward -= 0.5
        elif reward < -3:
            ...
            reward -= (self.success_count+1)*3*0.8+1
        ...
        print(self.success_count,reward)
        return reward
    ...
    def reset(self):
        ...
        # 翻转模式的状态
        # values=[0, 0, 0, 0, 180, 0,2500,80]
        # 失速模式的状态
        # values = [0,0,0,
        #           80,0,0,
        #           2500, 10]
        # # spin
        # values = [52.21, 48.52, 12.08,
        #           80, -48, 70.46,
        #           2500, 100]
        # # vel = [28.722, -54.959, -15.98, 52.21, 48.52, 12.08]

        values=[np.random.uniform(-90,90), np.random.uniform(-90,90), np.random.uniform(-90,90),
                np.random.uniform(-90,90),  np.random.uniform(-90,90), np.random.uniform(-90,90),
                # 80,0,0,
                2500, np.random.uniform(0,160)]
        # values = [np.random.uniform(-60, 60), np.random.uniform(-60, 60), np.random.uniform(-60, 60),
        #           np.random.uniform(-60, 60), np.random.uniform(-60, 60), np.random.uniform(-60, 60),
        #           # 80,0,0,
        #           2500, np.random.uniform(0, 160)]
        ...
        readWeight = XplaneEnv.CLIENT.getDREFs(self.ControlParameters.payloadWeight)
        print('weight_init:', readWeight[0])
        payloadWeight = [np.random.uniform(0,300)] # 负载重量  单位kg
        totalWeight = [np.random.uniform(781,1160)] # 飞机总重量 单位kg
        wind = [np.random.uniform(0,1),np.random.uniform(0,359),np.random.uniform(0,60*0.51444),np.random.uniform(0,60*0.51444),np.random.uniform(0,60*0.51444)]
        # XplaneEnv.CLIENT.sendDREFs(self.ControlParameters.payloadWeight, payloadWeight)  # 负重随机性
        # XplaneEnv.CLIENT.sendDREFs(self.ControlParameters.weatherWind, wind)  # 风的方向，速度和乱流随机性
        readWeight = XplaneEnv.CLIENT.getDREFs(self.ControlParameters.payloadWeight)
        print('weight:', readWeight[0])
        ...
        #XplaneEnv.CLIENT.sendDREF("sim/operation/failure/rel_wind_shear",1)
    ...
    def reset_spin(self):
        ...
        """
        XplaneEnv.CLIENT.sendCOMM('sim/operation/reset_flight')  # reset plane
        # XplaneEnv.CLIENT.sendCOMM('simiew')  # set the external view
        XplaneEnv.CLIENT.sendCOMM('sim/view/circle')  # set the external view
        ...
        init_position = ["sim/flightmodel/position/P",  # roll_rate
                          ...
                          "sim/flightmodel/position/phi",  # roll
                         "sim/flightmodel/position/theta",  # pitch
                          "sim/flightmodel/positioni",  # heading
                         "sim/cockpit/autopilot/altitude",
                         "sim/flightmodel/position/indicated_airspeed"]
        init_vel = ['sim/flightmodel/position/local_vx',
                   'sim/flightmodel/position/local_vy',
        ...
        # pos = [0,0, 2500, values[3], values[4], values[5], -998]
        pos = [-12.15,142.15,2500,-48,80,70.46,-998]
        XplaneEnv.CLIENT.sendDREFs(init_vel, vel)
        self.ControlParameters.stateAircraftPosition = XplaneEnv.CLIENT.sendPOSI(pos)   # reset the position
        self.ControlParameters.stateVariableValue = []
        self.ControlParameters.episodeReward = 0.
        self.ControlParameters.flag = False
