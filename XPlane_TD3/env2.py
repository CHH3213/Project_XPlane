...
class XplaneEnv(gym.Env):
    ...
    def reset(self):
        ...
        values=[np.random.uniform(-60,60), np.random.uniform(-60,60), np.random.uniform(-60,60),
                np.random.uniform(-60,60), np.random.uniform(-60,60), 0,
        ...
        wind = [np.random.uniform(0,1),np.random.uniform(0,359)]
        XplaneEnv.CLIENT.sendDREFs(self.ControlParameters.payloadWeight, payloadWeight)  # 负重随机性
        # XplaneEnv.CLIENT.sendDREFs(self.ControlParameters.weatherWind, wind)  # 风的方向和乱流随机性
        readWeight = XplaneEnv.CLIENT.getDREFs(self.ControlParameters.payloadWeight)
