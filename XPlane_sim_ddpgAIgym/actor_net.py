...
class ActorNet:
    ...
    def __init__(self,num_states,num_actions, sess, lr = LEARNING_RATE,batch_size=BATCH_SIZE):
        ...
        with self.g.as_default():
            ...
            self.optimizer = tf.train.AdamOptimizer(lr).apply_gradients(zip(self.parameters_gradients,self.actor_parameters))
    ...
    def create_actor_net(self, num_states=4, num_actions=1):
        ...
        # training = tf.placeholder_with_default(False, shape=(), name='training')  # 添加的batch normalization
        # actor_state_in =tf.layers.batch_normalization(actor_state_in,training=training,mementum=0.9)
        H1_a=tf.nn.relu(tf.matmul(actor_state_in,W1_a)+B1_a)
        # H1_a = tf.layers.batch_normalization(H1_a,training=training,mementum=0.9)
        # print(H1_a)
        H2_a=tf.nn.tanh(tf.matmul(H1_a,W2_a)+B2_a)
        # H2_a = tf.layers.batch_normalization(H2_a,training=training,mementum=0.9)
        actor_model=tf.matmul(H2_a,W3_a) + B3_a
        # actor_model=tf.nn.tanh(actor_model)
        # actor_model = tf.layers.batch_normalization(actor_model,training=training,mementum=0.9)
        # print(actor_model)
