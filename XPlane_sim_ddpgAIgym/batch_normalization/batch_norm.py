...
class batch_norm:
    def __init__(self,inputs,size,is_training,sess,parForTarget=None,bn_param=None):
        ...
        self.sess = sess        
        self.scale = tf.Variable(tf.random.uniform([size],0.9,1.1))
        self.beta = tf.Variable(tf.random.uniform([size],-0.03,0.03))
        self.pop_mean = tf.Variable(tf.random.uniform([size],-0.03,0.03),trainable=False)
        ...
        self.bnorm = tf.cond(is_training,training,testing)  # 类似if，else
