...
def s_norm(env, obs):
    ...
    for i in range(env.observation_space.shape[0]):
        # print(obs[i])
        obs_norm.append(-1. + (2) / (env.observation_space.high[i]-env.observation_space.low[i]) * (
...
def main():
    ...
    parser.add_argument('--saveData_dir', default="./save/xplane_ddpg_2/data",
                        help="directory to store all experiment data")
    parser.add_argument('--saveModel_dir', default='./save/xplane_ddpg_2',
                        help="where to store/load network weights")
    parser.add_argument('--load_dir', default='./save/xplane_ddpg_2',
    ...
    for episode in range(args.episodes):
        ...
        for t in range(max_steps):
            ...
            # action = tf.nn.tanh(action)
            # action = action.eval()
            VAR=1
            if episode >=50 and episode%10==0:
                VAR *=0.95  # 随着训练时间的增长，让action的探索性减弱
