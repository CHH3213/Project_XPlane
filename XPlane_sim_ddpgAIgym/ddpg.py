...
class DDPG:
    ...
    def __init__(self, env, is_batch_norm, BATCH_SIZE=64, REPLAY_MEMORY_SIZE=10000, sess=tf.InteractiveSession(),lr=LEARNING_RATE):
        self.env = env 
        ...
        if is_batch_norm:
            self.critic_net = CriticNet_bn(self.num_states, self.num_actions,sess,lr,self.batch_size)
            self.actor_net = ActorNet_bn(self.num_states, self.num_actions,sess,lr,self.batch_size)
            
        else:
            # pass
            self.critic_net = CriticNet(self.num_states, self.num_actions,sess,lr,self.batch_size)
            self.actor_net = ActorNet(self.num_states, self.num_actions,sess,lr,self.batch_size)
    ...
    def train(self):
        ...
        for i in range(0,self.batch_size):
            ...
            if self.done_batch[i]:
                ...
                self.y_i_batch.append(self.reward_batch[i] + GAMMA*q_t_1[i][0])  # 在这里将奖励用于目标网络               
        ...
        # print(action_for_delQ)
        if is_grad_inverter:        
            self.del_Q_a = self.critic_net.compute_delQ_a(self.state_t_batch,action_for_delQ)#/self.batch_size
            self.del_Q_a = self.grad_inv.invert(self.del_Q_a,action_for_delQ) 
        else:
            self.del_Q_a = self.critic_net.compute_delQ_a(self.state_t_batch,action_for_delQ)[0]#/self.batch_size
