...
class CriticNet:
    ...
    def __init__(self,num_states,num_actions,sess, lr = LEARNING_RATE, batch_size=BATCH_SIZE):
        ...
        with self.g.as_default():
            ...
            self.action_gradients = [self.act_grad_v[0]/tf.cast(tf.shape(self.act_grad_v[0])[0],dtype=float)] #this is just divided by batch size
            #from simple actor net:
    ...
    def create_critic_net(self, num_states=4, num_actions=1):
        ...
        # critic_state_in = tf.layers.batch_normalization(critic_state_in, training=training, mementum=0.9)

        H1_c=tf.nn.relu(tf.matmul(critic_state_in,W1_c)+B1_c)
        # H1_c = tf.layers.batch_normalization(H1_c, training=training, mementum=0.9)

        H2_c=tf.nn.tanh(tf.matmul(H1_c,W2_c)+tf.matmul(critic_action_in,W2_action_c)+B2_c)
        # H2_c = tf.layers.batch_normalization(H2_c, training=training, mementum=0.9)

        critic_q_model=tf.matmul(H2_c,W3_c)+B3_c
        # critic_q_model = tf.layers.batch_normalization(critic_q_model, training=training, mementum=0.9)
