...
class XplaneEnv(gym.Env):
    ...
    def __init__(self, clientAddr='0.0.0.0', xpHost='127.0.0.1', xpPort=49009, clientPort=1, timeout=1000):
        ...
        self.observation_space = spaces.Box(low=np.array([ -360, -360, -360 ,-180,-180,0,0,0]),high=np.array([360,360,360,180,180,360,5000,200]), dtype=np.float32)
    ...
    def rewardCalcul(self, targetState, xplaneState,action):
        ...
        if dist<=50:
            ...
            if self.success_count>50 or time.time()-start>1:  # 一个episode中有50step或者2秒连续处于平稳飞行状态时，认为success
                self.ControlParameters.flag = True
        ...
        # if -20 < xplaneState[4]-targetState[4] < 0 and action[0] < 0:
        #     reward += 0.01
        # elif -20 < xplaneState[4]-targetState[4] < 0 and action[0] > 0:
        #     reward +=-0.01
        # elif 0 < xplaneState[4]-targetState[4] < 100 and action[0] > 0:
        #     reward +=0.01
        # elif 0 < xplaneState[4]-targetState[4] < 100 and action[0] < 0:
        #     reward +=-0.01
        if -20 < xplaneState[4] - targetState[4] < 100:
            reward += 0.01
        if 100<xplaneState[1]<=180 or -180<xplaneState[1]<-100 or 100<xplaneState[2]<=180 or -180<xplaneState[2]<-100:  # 如果坠机或者翻过来了
            reward += -10
            self.success_count = 0
            self.extreme_count +=1
            if self.extreme_count>10 or time.time()-start>1:  # 连续10steps或1秒处于极端状态时，reset
                self.crash_flag=True
                print('extreme state!!!')
        # print('a step time', time.time() - start)
    ...
    def reset(self):
        ...
        XplaneEnv.CLIENT.sendCOMM('sim/view/circle')  # change the view
        XplaneEnv.CLIENT.sendCOMM('sim/instruments/copilot_DG_sync_mag')  #  magnetic north.
        ...
        XplaneEnv.CLIENT.sendDREFs(self.ControlParameters.velocityVariable,velocityValue)
        # vel = XplaneEnv.CLIENT.getDREFs(self.ControlParameters.velocityVariable)
        # print('vel',vel)
        pos=[-12.15,142.15,2500,np.random.uniform(-5,5),np.random.uniform(-5,5),np.random.uniform(-5,5),-998]
