...
######################################################################################################
    ...
    def setCONN(self, port):
        ...
        #Send command
        buffer = struct.pack(b"<4sxH", b"CONN", port)  # 按照给定的格式(fmt)，把数据封装成字符串(实际上是类似于c结构体的字节流)
        self.sendUDP(buffer)
...
###################################################################################################################
    ...
    def sendWYPT(self, op, points):
        """Adds, removes, or clears waypoints（航路点）. Waypoints are three dimensional points on or
